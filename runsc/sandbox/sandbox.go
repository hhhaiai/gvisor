...
func New(conf *config.Config, args *Args) (*Sandbox, error) {
	...
	// TODO(fvoznika): Use conf.EventHook as path to config file for now...
	if len(conf.EventHook) > 0 {
		config, err := os.Open(conf.EventHook)
		if err != nil {
			return nil, err
		}
		...
		for _, sink := range session.Sinks {
			...
			args.SinkFiles = append(args.SinkFiles, sinkFile)
		}
	}
...
func (s *Sandbox) createSandboxProcess(conf *config.Config, args *Args, startSyncFile *os.File) error {
	...
	if err := donations.OpenAndDonate("event-config-fd", conf.EventHook, os.O_RDONLY); err != nil {
		return err
	}
