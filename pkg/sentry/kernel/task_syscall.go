...
func (t *Task) executeSyscall(sysno uintptr, args arch.SyscallArguments) (rval uintptr, ctrl *SyscallControl, err error) {
	...
	if seccheck.Global.SyscallEnabled(seccheck.SyscallRawEnter, sysno) {
		...
		if !fields.Context.Empty() {
			...
			t.Credentials().LoadSeccheckInfo(fields.Context, info.Common)
			t.loadSeccheckInfoLocked(fields.Context, info.Common)
		}
	...
	if seccheck.Global.SyscallEnabled(seccheck.SyscallEnter, sysno) {
		...
		if cb != nil {
			...
			if !fields.Context.Empty() {
				common = &pb.Common{}
				t.Credentials().LoadSeccheckInfo(fields.Context, common)
				t.loadSeccheckInfoLocked(fields.Context, common)
			}
